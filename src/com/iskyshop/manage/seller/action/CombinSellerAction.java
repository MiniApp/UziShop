...
import com.iskyshop.foundation.domain.query.CombinLogQueryObject;
...
public class CombinSellerAction {
    ...
    public ModelAndView combin(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String orderBy, String orderType) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/combin.html", this.configService.getSysConfig(),
    ...
    public ModelAndView combin_log(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String orderBy, String orderType) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/combin_log.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
        CombinLogQueryObject qo = new CombinLogQueryObject(currentPage, mv, orderBy, orderType);
    ...
    public ModelAndView combin_apply(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/combin_apply.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
        ...
        if (store.getCombin_end_time() == null) {
            ...
            mv.addObject("url", CommUtil.getURL(request) + "/seller/combin_buy.htm");
            return mv;
        }
        ...
        Map map = CommUtil.cal_time_space(new Date(), store.getCombin_begin_time());
        ...
        request.getSession(false).setAttribute("combin_session", combin_session);
        return mv;
    }
    ...
    public ModelAndView combin_save(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String orderBy, String orderType, String combin_session, String combin_begin_time, String combin_end_time,
            String combin_main_goods_id, String combin_goods_ids, String combin_price, String id) {
        ModelAndView mv = new JModelAndView("success.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        String combin_session1 = CommUtil.null2String(request.getSession(false).getAttribute("combin_session"));
        ...
        if (combin_session1.equals(combin_session)) {
            ...
            Goods goods = this.goodsService.getObjById(CommUtil.null2Long(combin_main_goods_id));
            ...
            goods.setCombin_price(BigDecimal.valueOf(CommUtil.null2Float(combin_price)));
            for (String goods_id : combin_goods_ids.split(",")) {
                Goods combin_goods = this.goodsService.getObjById(CommUtil.null2Long(goods_id));
        ...
        mv.addObject("url", CommUtil.getURL(request) + "/seller/combin.htm?currentPage=" + currentPage);
        return mv;
    }
    ...
    public String delivery_del(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String mulitId) {
    ...
    public ModelAndView combin_buy(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/combin_buy.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
    ...
    public String combin_buy_save(HttpServletRequest request, HttpServletResponse response, String count) {
        ...
        int combin_gold = CommUtil.null2Int(count) * this.configService.getSysConfig().getCombin_amount();
        if (gold > combin_gold) {
    ...
    public ModelAndView combin_goods(HttpServletRequest request, HttpServletResponse response, String goods_name,
            String currentPage, String target_id, String goods_type) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/combin_goods.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        if (target_id.equals("main_goods_list")) {
            goods_type = "main";
        }
        if (CommUtil.null2String(goods_type).equals("main")) {
            mv = new JModelAndView("user/default/usercenter/combin_main_goods.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 0, request, response);
        }
        User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
        Store store = user.getStore();
        ...
        if (!(CommUtil.null2String(goods_name).equals(""))) {
            qo.addQuery("obj.goods_name", new SysMap("goods_name", "%" + CommUtil.null2String(goods_name) + "%"),
                    "like");
        }
        qo.addQuery("obj.combin_status", new SysMap("combin_status", Integer.valueOf(0)), "=");
        ...
        IPageList pList = this.goodsService.list(qo);
        CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/seller/combin_goods.htm", "", "&goods_name="
                + goods_name, pList, mv);
