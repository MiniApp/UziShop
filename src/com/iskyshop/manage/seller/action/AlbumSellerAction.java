...
public class AlbumSellerAction {
    ...
    public ModelAndView album(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/album.html", this.configService.getSysConfig(),
        ...
        CommUtil.saveIPageList2ModelAndView(url + "/seller/album.htm", "", "", pList, mv);
    ...
    public ModelAndView album_add(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/album_add.html",
    ...
    public ModelAndView album_edit(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String id) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/album_add.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
    ...
    public ModelAndView album_upload(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/album_upload.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        Map params = new HashMap();
        params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
        List objs = this.albumService.query(
    ...
    public String album_del(HttpServletRequest request, String mulitId) {
        ...
        for (String id : ids) {
            if (!(id.equals(""))) {
                List<Accessory> accs = this.albumService.getObjById(Long.valueOf(Long.parseLong(id))).getPhotos();
                for (Accessory acc : accs) {
                    CommUtil.del_acc(request, acc);
                    this.databaseTools.execute("update iskyshop_album set album_cover_id=null where album_cover_id="
                            + acc.getId());
                }
    ...
    public String album_cover(HttpServletRequest request, String album_id, String id, String currentPage) {
        Accessory album_cover = this.accessoryService.getObjById(Long.valueOf(Long.parseLong(id)));
    ...
    public ModelAndView album_transfer(HttpServletRequest request, HttpServletResponse response, String currentPage,
        ...
        ModelAndView mv = new JModelAndView("user/default/usercenter/album_transfer.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        Map params = new HashMap();
        params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
        List objs = this.albumService.query(
    ...
    public String album_transfer_save(HttpServletRequest request, String mulitId, String album_id, String to_album_id,
        ...
        for (String id : ids) {
            if (!(id.equals(""))) {
                Accessory acc = this.accessoryService.getObjById(Long.valueOf(Long.parseLong(id)));
                Album to_album = this.albumService.getObjById(Long.valueOf(Long.parseLong(to_album_id)));
    ...
    public ModelAndView album_image(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/album_image.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        ...
        aqo.addQuery("obj.album.user.id", new SysMap("user_id", SecurityUserHolder.getCurrentUser().getId()), "=");
        aqo.setCurrentPage(Integer.valueOf(CommUtil.null2Int(currentPage)));
        ...
        CommUtil.saveIPageList2ModelAndView(url + "/seller/album_image.htm", "", "&id=" + id, pList, mv);
        Map params = new HashMap();
        params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
        List albums = this.albumService.query(
                "select obj from Album obj where obj.user.id=:user_id order by obj.album_sequence asc", params, -1, -1);
    ...
    public ModelAndView image_slide(HttpServletRequest request, HttpServletResponse response, String album_id, String id) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/image_slide.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
    ...
    public String album_watermark(HttpServletRequest request, String mulitId, String album_id, String to_album_id,
        ...
        if (store_id != null) {
            WaterMark waterMark = this.waterMarkService.getObjByProperty("store.id", store_id);
            if (waterMark != null) {
                String[] ids = mulitId.split(",");
                for (String id : ids) {
                    if (!(id.equals(""))) {
                        ...
                        if (waterMark.isWm_image_open()) {
                            String wm_path = request.getSession().getServletContext().getRealPath("/")
                                    + waterMark.getWm_image().getPath() + File.separator
                        ...
                        if (waterMark.isWm_text_open()) {
                            Font font = new Font(waterMark.getWm_text_font(), 1, waterMark.getWm_text_font_size());
                            CommUtil.waterMarkWithText(path, path, waterMark.getWm_text(),
                                    waterMark.getWm_text_color(), font, waterMark.getWm_text_pos(), 100.0F);
                        }
                    }
                }
            }
