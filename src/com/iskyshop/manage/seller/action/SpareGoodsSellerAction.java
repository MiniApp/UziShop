...
import com.iskyshop.foundation.domain.SpareGoods;
import com.iskyshop.foundation.domain.SpareGoodsClass;
import com.iskyshop.foundation.domain.SysConfig;
import com.iskyshop.foundation.domain.User;
import com.iskyshop.foundation.domain.query.SpareGoodsQueryObject;
import com.iskyshop.foundation.service.IAccessoryService;
import com.iskyshop.foundation.service.IAreaService;
import com.iskyshop.foundation.service.ISpareGoodsClassService;
import com.iskyshop.foundation.service.ISpareGoodsService;
...
public class SpareGoodsSellerAction {
    ...
    @SecurityMapping(title = "闲置商品列表", value = "/seller/spare_goods.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    ...
    public ModelAndView spare_goods(HttpServletRequest request, HttpServletResponse response, String currentPage,
        ...
        ModelAndView mv = new JModelAndView("user/default/usercenter/spare_goods.html",
        ...
        qo.addQuery("obj.user.id", new SysMap("obj_user_id", SecurityUserHolder.getCurrentUser().getId()), "=");
        if ((type != null) && (!(type.equals("")))) {
            qo.addQuery("obj.down", new SysMap("obj_type", Integer.valueOf(CommUtil.null2Int(type))), "=");
    ...
    @SecurityMapping(title = "发布闲置商品", value = "/seller/add_spare_goods.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    ...
    public ModelAndView spare_goods_add(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/add_spare_goods.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        Map map = new HashMap();
        map.put("level", Integer.valueOf(2));
        List level2 = this.sparegoodsclassService.query(
                "select obj from SpareGoodsClass obj where obj.level=:level order by sequence asc", map, -1, -1);
    ...
    @SecurityMapping(title = "闲置商品信息编辑", value = "/seller/spare_goods_edit.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    ...
    public ModelAndView spare_goods_edit(HttpServletRequest request, HttpServletResponse response, String id) {
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            mv = new JModelAndView("user/default/usercenter/add_spare_goods.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 0, request, response);

            List sgcs = this.sparegoodsclassService.query(
                    "select obj from SpareGoodsClass obj where obj.parent.id is null order by sequence asc", null, -1,
                    -1);
            Map map = new HashMap();
            map.put("level", Integer.valueOf(2));
            List level2 = this.sparegoodsclassService.query(
    ...
    @SecurityMapping(title = "闲置商品信息删除", value = "/seller/spare_goods_dele.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({ "/seller/spare_goods_dele.htm" })
    public ModelAndView spare_goods_dele(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage) {
    ...
    @SecurityMapping(title = "闲置商品下架上架操作", value = "/seller/spare_goods_updown.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({ "/seller/spare_goods_updown.htm" })
    public ModelAndView spare_goods_updown(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage, String type) {
        ModelAndView mv = new JModelAndView("error", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 0, request, response);
        mv.addObject("op_title", "您所访问的地址不存在!");
        mv.addObject("url", CommUtil.getURL(request) + "/seller/spare_goods.htm?currentPage=" + currentPage);
        SpareGoods obj = this.sparegoodsService.getObjById(CommUtil.null2Long(id));
        if ((obj != null) && (!(obj.equals("")))
                && (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId()))) {
            obj.setDown(CommUtil.null2Int(type));
            this.sparegoodsService.update(obj);
            mv = new JModelAndView("user/default/usercenter/success.html", this.configService.getSysConfig(),
                    this.userConfigService.getUserConfig(), 0, request, response);
    ...
    @SecurityMapping(title = "闲置商品保存", value = "/seller/spare_goods_save.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({ "/seller/spare_goods_save.htm" })
    public ModelAndView spare_goods_save(HttpServletRequest request, HttpServletResponse response, String id,
            String class_id, String oldAndnew, String area_id, String img1_id, String img2_id, String img3_id,
            String img4_id, String img5_id, String main_img_id, String spare_goods_session) {
        ModelAndView mv = null;
        String spare_goods_session1 = CommUtil.null2String(request.getSession(false)
                .getAttribute("spare_goods_session"));
        if (spare_goods_session.equals(spare_goods_session1)) {
            WebForm wf = new WebForm();
            SpareGoods sparegoods = null;
            SpareGoodsClass goodsClass = this.sparegoodsclassService.getObjById(CommUtil.null2Long(class_id));
            if (id.equals("")) {
            ...
            if ((img1_id != null) && (!(img1_id.equals("")))) {
                Accessory img1 = this.accessoryService.getObjById(CommUtil.null2Long(img1_id));
                sparegoods.setImg1(img1);
            }
            if ((img2_id != null) && (!(img2_id.equals("")))) {
                Accessory img2 = this.accessoryService.getObjById(CommUtil.null2Long(img2_id));
                sparegoods.setImg2(img2);
            }
            if ((img3_id != null) && (!(img3_id.equals("")))) {
                Accessory img3 = this.accessoryService.getObjById(CommUtil.null2Long(img3_id));
                sparegoods.setImg3(img3);
            }
            if ((img4_id != null) && (!(img4_id.equals("")))) {
                Accessory img4 = this.accessoryService.getObjById(CommUtil.null2Long(img4_id));
                sparegoods.setImg4(img4);
            }
            if ((img5_id != null) && (!(img5_id.equals("")))) {
                Accessory img5 = this.accessoryService.getObjById(CommUtil.null2Long(img5_id));
                sparegoods.setImg5(img5);
            }
            if ((main_img_id != null) && (!(main_img_id.equals("")))) {
                Accessory main_img = this.accessoryService.getObjById(CommUtil.null2Long(main_img_id));
    ...
    @SecurityMapping(title = "闲置商品发布页Ajax加载下级地区信息", value = "/seller/sparegoods_area_data.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({ "/seller/sparegoods_area_data.htm" })
    public ModelAndView sparegoods_area_data(HttpServletRequest request, HttpServletResponse response,
            String parent_id, String area_mark) {
        ModelAndView mv = new JModelAndView("user/default/usercenter/sparegoods_area_data.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
        Map map = new HashMap();
    ...
    @SecurityMapping(title = "图片删除", value = "/seller/sparegoods_removeimg.htm*", rtype = "seller", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({ "/seller/sparegoods_removeimg.htm" })
    public void sparegoods_img_dele(HttpServletRequest request, HttpServletResponse response, String count,
            String sp_id, String img_id) {
        SpareGoods sp = null;
        Accessory img = this.accessoryService.getObjById(CommUtil.null2Long(img_id));
        if ((sp_id != null) && (!(sp_id.equals("")))) {
            sp = this.sparegoodsService.getObjById(CommUtil.null2Long(sp_id));
            if ((count.equals("1")) && (sp.getImg1() != null) && (!(sp.getImg1().equals("")))) {
                sp.setImg1(null);
            }

            if ((count.equals("2")) && (sp.getImg2() != null) && (!(sp.getImg2().equals("")))) {
                sp.setImg2(null);
            }

            if ((count.equals("3")) && (sp.getImg3() != null) && (!(sp.getImg3().equals("")))) {
                sp.setImg3(null);
            }

            if ((count.equals("4")) && (sp.getImg4() != null) && (!(sp.getImg4().equals("")))) {
                sp.setImg4(null);
            }

            if ((count.equals("5")) && (sp.getImg5() != null) && (!(sp.getImg5().equals("")))) {
                sp.setImg5(null);
            }
