  ...
  implements ServletContextAware
{
  ...
  public ModelAndView admin_list(String currentPage, String orderBy, String orderType, HttpServletRequest request, HttpServletResponse response)
  {
    ...
    uqo.addQuery("obj.userRole", 
      new SysMap("userRole1", 
    ...
    for (String id : ids) {
      if (!(id.equals(""))) {
        ...
        if (!(user.getUsername().equals("admin"))) {
          this.databaseTools.execute("delete from iskyshop_syslog where user_id=" + 
            id);
          this.databaseTools.execute("delete from iskyshop_user_role where user_id=" + 
            id);
  ...
  public String init_role()
  {
    ...
    if ((current_user != null) && 
      ...
      List buyers = this.userService.query(
        "select obj from User obj where obj.userRole=:userRole", 
        (Map)params, -1, -1);
      //suhao for (User buyer : buyers) {
      for (int i = 0; i < buyers.size(); i++) {
        User buyer = (User) buyers.get(i);
      ...
      List seller_roles = this.roleService
