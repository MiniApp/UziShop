...
import com.iskyshop.foundation.service.IMobileVerifyCodeService;
...
public class StatManageAction {
    ...
    private void execute() throws Exception {
        ...
        List<MobileVerifyCode> mvcs = this.mobileverifycodeService.query("select obj from MobileVerifyCode obj where obj.addTime<=:time",
                params, -1, -1);
        ...
        for (StoreClass sc : scs) {
            ...
            List<StorePoint> sp_list = this.storePointService.query("select obj from StorePoint obj where obj.store.sc.id=:sc_id",
                    params, -1, -1);
        ...
        for (Group group : groups) {
            ...
            if (group.getEndTime().before(new Date())) {
                group.setStatus(-2);
                this.groupService.update(group);
                for (Iterator<GroupGoods> it = group.getGg_list().iterator(); it.hasNext();) {
                    gg = it.next();
                    gg.setGg_status(-2);
        ...
        for (Activity act : acts) {
            ...
            for (ActivityGoods ac : act.getAgs()) {
                ...
                Goods goodsq = ac.getAg_goods();
                goodsq.setActivity_status(0);
                goodsq.setGoods_current_price(goodsq.getStore_price());
                this.goodsService.update(goodsq);
            }
        }
        ...
        params.put("shipTime", cal.getTime());
        List<OrderForm> confirm_ofs = this.orderFormService.query("select obj from OrderForm obj where obj.shipTime<=:shipTime",
                params, -1, -1);
        OrderFormLog ofl;
        PredepositLog log;
        User buyer;
            ...
            if (ret) {
                ...
                if (of.getPayment().getMark().equals("balance")) {
                    ...
                    if (this.configService.getSysConfig().getBalance_fenrun() == 1) {
                        ...
                        PredepositLog log2 = new PredepositLog();
                        log2.setAddTime(new Date());
                        log2.setPd_log_user(seller);
                        log2.setPd_op_type("分润");
                        log2.setPd_log_amount(BigDecimal.valueOf(shop_availableBalance));
                        log2.setPd_log_info("自动确认收货平台分润获得预存款，订单" + of.getOrder_id());
                        log2.setPd_type("可用预存款");
                        this.predepositLogService.save(log2);
                        ...
                        log1.setPd_log_info("自动确认收货增加预存款，订单号" + of.getOrder_id());
                        log1.setPd_type("可用预存款");
                        this.predepositLogService.save(log1);

                        User buyer2 = of.getUser();
                        buyer2.setFreezeBlance(BigDecimal.valueOf(CommUtil.subtract(buyer2.getFreezeBlance(),
                                of.getTotalPrice())));
                        this.userService.update(buyer2);
                    } else {
                        seller.setAvailableBalance(BigDecimal.valueOf(CommUtil.add(seller.getAvailableBalance(),
                                of.getTotalPrice())));
                        this.userService.update(seller);
                        ...
                        buyer = of.getUser();
                        buyer.setFreezeBlance(BigDecimal.valueOf(CommUtil.subtract(buyer.getFreezeBlance(),
                                of.getTotalPrice())));
        ...
        List<OrderForm> confirm_evaluate_ofs = this.orderFormService
                .query("select obj from OrderForm obj where obj.return_shipTime<=:return_shipTime and obj.order_status>=:order_status_40 and obj.order_status!=:order_status_47 and obj.order_status!=:order_status_48 and obj.order_status!=:order_status_49 and obj.order_status!=:order_status_50 and obj.order_status!=:order_status_60",
                        params, -1, -1);
        ...
        for (DeliveryGoods dg : dgs) {
            ...
            Goods goods3 = dg.getD_goods();
            goods3.setDelivery_status(0);
        ...
        for (Store store : stores) {
            //for (ship_evaluate = store.getGoods_list().iterator(); ship_evaluate.hasNext();) {
            for (Iterator it = store.getGoods_list().iterator(); it.hasNext();) {
                Goods goods4 = (Goods) it.next();
        ...
        List<Goods> goods_list = this.evaluateService.query_goods("select distinct obj.evaluate_goods from Evaluate obj ",
                null, -1, -1);
        
        for (Goods goods2 : goods_list) {
            double description_evaluate2 = 0.0D;
            params.clear();
            params.put("evaluate_goods_id", goods2.getId());
            ...
            for (Evaluate eva : eva_list) {
                description_evaluate2 = CommUtil
                        .add(eva.getDescription_evaluate(), Double.valueOf(description_evaluate2));
            }

            description_evaluate2 = CommUtil.div(Double.valueOf(description_evaluate2), Integer.valueOf(eva_list.size()));
            goods2.setDescription_evaluate(BigDecimal.valueOf(description_evaluate2));
            this.goodsService.update(goods2);
        }

        params.clear();
        params.put("weixin_status", Integer.valueOf(1));
        List<Store> store_list = this.storeService.query("select obj from Store obj where obj.weixin_status=:weixin_status",
                params, -1, -1);
    ...
    private boolean send_email(OrderForm order, String mark) throws Exception {
        ...
        if (template.isOpen()) {
            ...
            String path = System.getProperty("iskyshop.root") + "vm" + File.separator;
            ...
            p.setProperty("file.resource.loader.path", System.getProperty("iskyshop.root") + "vm" + File.separator);
