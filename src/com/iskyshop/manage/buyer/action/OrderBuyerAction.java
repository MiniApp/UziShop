...
public class OrderBuyerAction {
            ...
            if (ret) {
                ...
                if (obj.getPayment().getMark().equals("balance")) {
                    ...
                    if (this.configService.getSysConfig().getBalance_fenrun() == 1) {
                        ...
                        double shop_availableBalance = CommUtil.null2Double(obj.getTotalPrice())
                        ...
                        double seller_availableBalance = CommUtil.null2Double(obj.getTotalPrice())
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            if (obj.getOrder_status() == 40) {
                ...
                for (GoodsCart gc : obj.getGcs()) {
                    ...
                    eva.setShip_evaluate(BigDecimal.valueOf(CommUtil.null2Double(request.getParameter("ship_evaluate"
                            + gc.getId()))));
                    ...
                    List sps = this.storePointService.query(
    ...
    public String order_delete(HttpServletRequest request, HttpServletResponse response, String id, String currentPage)
        ...
        if ((obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) && (obj.getOrder_status() == 0)) {
            for (GoodsCart gc : obj.getGcs()) {
                gc.getGsps().clear();
    ...
    public ModelAndView order_return_apply(HttpServletRequest request, HttpServletResponse response, String id,
            String view) {
    ...
    public String order_return_apply_save(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage, String return_content) throws Exception {
        OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
        ...
        if (obj.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
            ...
            if (this.configService.getSysConfig().isSmsEnbale()) {
                send_sms(request, obj, obj.getUser().getMobile(), "sms_toseller_order_return_apply_notify");
            }
        }
        return "redirect:order.htm?currentPage=" + currentPage;
    }
    ...
    public ModelAndView order_return_ship(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage) {
    ...
    public String order_return_ship_save(HttpServletRequest request, HttpServletResponse response, String id,
            String currentPage, String ec_id, String return_shipCode) {
