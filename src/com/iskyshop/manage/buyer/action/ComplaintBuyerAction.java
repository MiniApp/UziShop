...
public class ComplaintBuyerAction {
    ...
    public ModelAndView complaint(HttpServletRequest request, HttpServletResponse response, String currentPage,
            String status) {
        ...
        ComplaintQueryObject qo = new ComplaintQueryObject(currentPage, mv, "addTime", "desc");
        qo.addQuery("obj.from_user.id", new SysMap("user_id", SecurityUserHolder.getCurrentUser().getId()), "=");
        if (!(CommUtil.null2String(status).equals(""))) {
            qo.addQuery("obj.status", new SysMap("status", Integer.valueOf(CommUtil.null2Int(status))), "=");
        }
    ...
    public ModelAndView complaint_handle(HttpServletRequest request, HttpServletResponse response, String order_id) {
        ...
        if (of.getComplaints().size() > 0) {
            for (Complaint complaint : of.getComplaints()) {
                if (complaint.getFrom_user().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
                    result1 = false;
                }
            }
        }
        if (result) {
            if (result1) {
                ...
                List css = this.complaintSubjectService.query(
                        "select obj from ComplaintSubject obj where obj.type=:type", (Map) params, -1, -1);
    ...
    public ModelAndView complaint_save(HttpServletRequest request, HttpServletResponse response, String order_id,
    ...
    public void complaint_talk(HttpServletRequest request, HttpServletResponse response, String id, String talk_content)
            throws IOException {
        Complaint obj = this.complaintService.getObjById(CommUtil.null2Long(id));
        if (!(CommUtil.null2String(talk_content).equals(""))) {
            ...
            String temp = user_role + "[" + SecurityUserHolder.getCurrentUser().getUsername() + "] "
    ...
    public ModelAndView complaint_arbitrate(HttpServletRequest request, HttpServletResponse response, String id,
