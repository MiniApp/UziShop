...
public class UCClient extends PHPFunctions {
    ...
    static {
        InputStream in = UCClient.class.getClassLoader().getResourceAsStream("config.properties");
    ...
    public String uc_api_post(String module, String action, Map<String, Object> arg) {
        ...
        for (String k : arg.keySet()) {
            ...
            if (v.getClass().isAssignableFrom(Map.class)) {
                String s2 = "";
                String sep2 = "";
                Map<String, String> c = (Map) v;
                for (String k2 :  c.keySet()) {
                    Object v2 = ((Map) v).get(k2);
                    k2 = urlencode(k2);
                    s2 = s2 + sep2 + "{" + k + "}[" + k2 + "]=" + urlencode(uc_stripslashes(String.valueOf(v2)));
                    sep2 = "&";
                }
    ...
    protected String uc_api_requestdata(String module, String action, String arg, String extra) {
        ...
        String post = "m=" + module + "&a=" + action + "&inajax=2&release=" + UC_CLIENT_RELEASE + "&input=" + input
                + "&appid=" + UC_APPID + extra;
        return post;
    }
    ...
    public String uc_authcode(String string, String operation, String key, int expiry) {
        ...
        String keyc = (ckey_length > 0) ? substr(md5(microtime()), -ckey_length)
                : (operation.equals("DECODE")) ? substr(string, 0, ckey_length) : "";

        String cryptkey = keya + md5(new StringBuilder(String.valueOf(keya)).append(keyc).toString());
        int key_length = cryptkey.length();

        string = sprintf("%010d", (expiry > 0) ? expiry + time() : 0L)
        ...
        if (operation.equals("DECODE")) {
            ...
            if ((((CommUtil.null2Int(substr(result.toString(), 0, 10)) == 0) || (CommUtil.null2Long(
                    substr(result.toString(), 0, 10)).longValue()
                    - time() > 0L)))
                    && (substr(result.toString(), 10, 16).equals(substr(md5(substr(result.toString(), 26) + keyb), 0,
                            16)))) {
    ...
    protected String uc_fopen2(String url, int limit, String post, String cookie, boolean bysocket, String ip,
            int timeout, boolean block) {
        url = url + ((url.indexOf("?") > 0) ? "&" : "?__times__=1");
        return uc_fopen(url, limit, post, cookie, bysocket, ip, timeout, block);
    }

    protected String uc_fopen(String url, int limit, String post, String cookie, boolean bysocket, String ip,
            int timeout, boolean block) {
        String ret = "";
    ...
    public String uc_user_login(String username, String password, int isuid, int checkques, int questionid,
            String answer) {
        Map args = new HashMap();
        args.put("username", username);
        args.put("password", password);
    ...
    public String uc_user_synlogout() {
        String ret = uc_api_post("user", "synlogout", new HashMap());
        return ret;
    }
    ...
    public String uc_user_edit(String username, String oldpw, String newpw, String email, int ignoreoldpw,
            int questionid, int answer) {
        Map args = new HashMap();
        args.put("username", username);
