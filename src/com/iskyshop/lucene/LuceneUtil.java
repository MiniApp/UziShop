...
public class LuceneUtil
{
  ...
  public LuceneUtil()
  {
    ...
    parser = new ShopQueryParser(Version.LUCENE_35, "title", 
      analyzer);
  }
  ...
  private Document builderDocument(LuceneVo luceneVo)
  {
    ...
    Field content = new Field("content", Jsoup.clean(
      luceneVo.getVo_content(), Whitelist.none()), Field.Store.YES, 
      Field.Index.ANALYZED);
    Field type = new Field("type", luceneVo.getVo_type(), 
      Field.Store.YES, Field.Index.ANALYZED);
    NumericField store_price = new NumericField("store_price", 
      Field.Store.YES, true);
    store_price.setDoubleValue(luceneVo.getVo_store_price());
    Field add_time = new Field("add_time", 
      CommUtil.null2String(Long.valueOf(luceneVo.getVo_add_time())), 
      Field.Store.YES, Field.Index.ANALYZED);
    Field goods_salenum = new Field("goods_salenum", 
      CommUtil.null2String(Integer.valueOf(luceneVo.getVo_goods_salenum())), 
      Field.Store.YES, Field.Index.ANALYZED);
    document.add(id);
    document.add(title);
