...
public class CartViewAction {
    ...
    private List<StoreCart> cart_calc(HttpServletRequest request) {
        ...
        for (Iterator<StoreCart> it = cookie_cart.iterator(); it.hasNext();) {
            ...
            int l = 1;
            for (StoreCart sc1 : cart) {
                if (sc1.getStore() != null && sc.getStore() != null
    ...
    public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count,
            ...
            for (GoodsCart gc : sc3.getGcs())
                if ((gsp_ids != null) && (gsp_ids.length > 0) && (gc.getGsps() != null) && (gc.getGsps().size() > 0)) {
                    ...
                    for (int i = 0; i < gc.getGsps().size(); ++i) {
                        gsp_ids1[i] = ((gc.getGsps().get(i) != null) ? ((GoodsSpecProperty) gc.getGsps().get(i))
                                .getId().toString() : "");
                    }
        ...
        if (add) {
            ...
            for (StoreCart sc1 : cart) {
                if (sc1.getStore() != null && sc1.getStore().getId().equals(goods.getGoods_store().getId())) {
                    sc3 = sc1;
                    type = "update";
                    break;
                }
            ...
            for (Iterator<GoodsCart> it = sc.getGcs().iterator(); it.hasNext();) {
                ...
                if (CommUtil.null2String(gc1.getCart_type()).equals("")) {
                    cart_total_price = cart_total_price + CommUtil.null2Double(gc1.getGoods().getGoods_current_price())
                            * gc1.getCount();
                }
                if (!(CommUtil.null2String(gc1.getCart_type()).equals("combin")))
                    continue;
                cart_total_price = cart_total_price + CommUtil.null2Double(gc1.getGoods().getCombin_price())
                        * gc1.getCount();
            }

            sc.setTotal_price(BigDecimal.valueOf(CommUtil.formatMoney(Double.valueOf(cart_total_price))));
            if (user == null)
        ...
        for (StoreCart sc2 : cart) {
            for (GoodsCart gc1 : sc2.getGcs()) {
                ...
                if ((store_id == null) || (store_id.equals(""))
                        || (sc2.getStore() != null && !(sc2.getStore().getId().toString().equals(store_id))))
                    continue;
    ...
    public void goods_count_adjust(HttpServletRequest request, HttpServletResponse response, String cart_id,
        ...
        for (Iterator<StoreCart> it = cart.iterator(); it.hasNext();) {
            ...
            if (sc.getStore() != null && sc.getStore().getId().equals(CommUtil.null2Long(store_id))) {
                ((Map) map).put("sc_total_price", Float.valueOf(CommUtil.null2Float(sc.getTotal_price())));
            }
        }
    ...
    public ModelAndView goods_cart1(HttpServletRequest request, HttpServletResponse response) {
        ...
        List<StoreCart> cart = cart_calc(request);
        if (cart != null) {
            Store store = (SecurityUserHolder.getCurrentUser().getStore() != null) ? SecurityUserHolder
                    .getCurrentUser().getStore() : null;
            if (store != null) {
                for (StoreCart sc : cart) {
    ...
    public ModelAndView goods_cart2(HttpServletRequest request, HttpServletResponse response, String store_id) {
        ...
        if (cart != null) {
            for (StoreCart sc1 : cart) {
                if (sc1.getStore() != null && sc1.getStore().getId().equals(CommUtil.null2Long(store_id))) {
                    sc = sc1;
                    break;
                }
            }
        ...
        if (of.getOrder_status() == 10) {
            if (CommUtil.null2String(payType).equals("")) {
                ...
                if (payType.equals("balance")) {
                    ...
                    mv.addObject("paymentTools", this.paymentTools);
                    mv.addObject("store_id", this.orderFormService.getObjById(CommUtil.null2Long(order_id)).getStore()
                            .getId());
    ...
    public String cart_address_save(HttpServletRequest request, HttpServletResponse response, String id,
            String area_id, String store_id) {
        WebForm wf = new WebForm();
