...
public class CartViewAction {
	...
	private List<StoreCart> cart_calc(HttpServletRequest request) {
				...
				if (user.getStore() != null) {
					...
					List<StoreCart> store_cookie_cart = this.storeCartService
							.query("select obj from StoreCart obj where (obj.cart_session_id=:cart_session_id or obj.user.id=:user_id) and obj.sc_status=:sc_status and obj.store.id=:store_id",
									params, -1, -1);
					for (Iterator<StoreCart> localIterator1 = store_cookie_cart
		...
		// suhao for (Object sc = user_cart.iterator(); ((Iterator)
		// sc).hasNext();) {
	...
	public void add_goods_cart(HttpServletRequest request,
		...
		if (add) {
			...
			if (CommUtil.null2String(buy_type).equals("")) {
				...
				((GoodsCart) obj).setPrice(BigDecimal.valueOf(CommUtil
						.null2Double(price)));
			}
			...
			for (Iterator<GoodsCart> it = sc.getGcs().iterator(); it.hasNext();) {
				...
				cart_total_price = cart_total_price
						+ CommUtil
								.null2Double(gc1.getGoods().getCombin_price())
						* gc1.getCount();
			}

			sc.setTotal_price(BigDecimal.valueOf(CommUtil.formatMoney(Double
					.valueOf(cart_total_price))));
			if (user == null)
		...
		for (StoreCart sc2 : cart) {
			for (GoodsCart gc1 : sc2.getGcs()) {
				...
				sc_total_price = sc_total_price
						+ CommUtil.null2Double(gc1.getPrice()) * gc1.getCount();
				...
				if (gg.getGg_count() >= CommUtil.null2Int(count))
					...
					for (Iterator<StoreCart> it = cart.iterator(); it.hasNext();) {
						...
						for (int i = 0; i < ((StoreCart) sc).getGcs().size(); ++i) {
							...
							if (gc.getId().toString().equals(cart_id)) {
								((StoreCart) sc)
										.setTotal_price(BigDecimal.valueOf(CommUtil.add(
												((StoreCart) sc)
														.getTotal_price(),
												Double.valueOf((CommUtil
														.null2Int(count) - gc
														.getCount())
						...
						if (gc.getId().toString().equals(cart_id)) {
							sc.setTotal_price(BigDecimal.valueOf(CommUtil.add(
									...
									Double.valueOf((CommUtil.null2Int(count) - gc
											.getCount())
											* Double.parseDouble(gc.getPrice()
													.toString())))));
							gc.setCount(CommUtil.null2Int(count));
							gc1 = gc;
							sc.getGcs().remove(gc);
		...
		if (cart != null) {
			if (CommUtil.null2String(cart_session1).equals(cart_session)) {
				...
				if (!(CommUtil.null2String(coupon_id).equals(""))) {
					CouponInfo ci = this.couponInfoService.getObjById(CommUtil
							.null2Long(coupon_id));
					ci.setStatus(1);
			...
			if (ret) {
				...
				for (GoodsCart gc : of.getGcs()) {
					...
					if (goods.getInventory_type().equals("all")) {
						...
						for (Iterator localIterator4 = list.iterator(); localIterator4
								.hasNext();) {
	...
	public void order_address(HttpServletRequest request,
		...
		List<StoreCart> cart = (List) request.getSession(false).getAttribute(
				"cart");
		StoreCart sc = null;
		if (cart != null) {
			for (StoreCart sc1 : cart) {
	...
	private void send_email(HttpServletRequest request, OrderForm order,
		...
		com.iskyshop.foundation.domain.Template template = this.templateService
