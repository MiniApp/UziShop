...
import com.iskyshop.view.web.tools.SpareGoodsViewTools;
...
public class SpareGoodsViewAction {
    ...
    public ModelAndView sparegoods_head(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new JModelAndView("sparegoods_head.html", this.configService.getSysConfig(),
    ...
    public ModelAndView sparegoods_nav(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ModelAndView mv = new JModelAndView("sparegoods_nav.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        List sgcs = this.sparegoodsclassService.query(
                "select obj from SpareGoodsClass obj where obj.parent.id is null order by sequence asc", null, -1, -1);
        ...
        if ((SecurityUserHolder.getCurrentUser() != null) && (!(SecurityUserHolder.getCurrentUser().equals("")))) {
            ...
            List sps = this.sparegoodsService.query(
            ...
            params.put("uid", SecurityUserHolder.getCurrentUser().getId());
            List drops = this.sparegoodsService.query(
                    "select obj from SpareGoods obj where obj.status=:status and obj.user.id=:uid", params, -1, -1);

            params.clear();
            params.put("down", Integer.valueOf(-1));
            params.put("uid", SecurityUserHolder.getCurrentUser().getId());
            List down = this.sparegoodsService.query(
                    "select obj from SpareGoods obj where obj.down=:down and obj.user.id=:uid", params, -1, -1);
        ...
        List navs = this.navService.query(
    ...
    public ModelAndView sparegoods_nav2(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ModelAndView mv = new JModelAndView("sparegoods_nav2.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        List sgcs = this.sparegoodsclassService.query(
                "select obj from SpareGoodsClass obj where obj.parent.id is null order by sequence asc", null, -1, -1);
        mv.addObject("sgcs", sgcs);
    ...
    public ModelAndView sparegoods(HttpServletRequest request, HttpServletResponse response, String currentPage) {
        ...
        Map map = new HashMap();
        map.put("display", Boolean.valueOf(true));
        List floors = this.sparegoodsfloorService.query(
    ...
    public ModelAndView sparegoods_detail(HttpServletRequest request, HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("sparegoods_detail.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
    ...
    public ModelAndView sparegoods_search(HttpServletRequest request, HttpServletResponse response, String cid,
            String orderBy, String orderType, String currentPage, String price_begin, String price_end, String keyword,
            String area_id) {
        ModelAndView mv = new JModelAndView("sparegoods_search.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        ...
        if ((orderBy != null) && (!(orderBy.equals("")))) {
            ...
            if ((price_end != null) && (!(price_end.equals("")))) {
                qo.addQuery("obj.goods_price", new SysMap("goods_end", Integer.valueOf(CommUtil.null2Int(price_end))),
                        "<=");
            }
        }
        ...
        List citys = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
