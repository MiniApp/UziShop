...
public class VerifyAction {
    ...
    public void verify_email(HttpServletRequest request, HttpServletResponse response, String email, String id) {
        ...
        params.put("id", CommUtil.null2Long(id));
        List users = this.userService.query("select obj from User obj where obj.email=:email and obj.id!=:id", params,
                -1, -1);
        if ((users != null) && (users.size() > 0)) {
            ret = false;
        }
    ...
    public void verify_storename(HttpServletRequest request, HttpServletResponse response, String store_name, String id) {
        ...
        params.put("id", CommUtil.null2Long(id));
        List users = this.storeService.query(
                "select obj from Store obj where obj.store_name=:store_name and obj.id!=:id", params, -1, -1);
        if ((users != null) && (users.size() > 0)) {
            ret = false;
        }
        response.setContentType("text/plain");
    ...
    public void verify_mobile(HttpServletRequest request, HttpServletResponse response, String mobile, String id) {
        ...
        params.put("id", CommUtil.null2Long(id));
        List users = this.userService.query("select obj from User obj where obj.mobile=:mobile and obj.id!=:id",
                params, -1, -1);
        if ((users != null) && (users.size() > 0)) {
            ret = false;
        }
