...
public class StoreViewTools {
    ...
    public int generic_store_credit(String id) {
        ...
        for (int i = 0; i < ints.length - 1; ++i) {
            if ((ints[i].intValue() > store.getStore_credit()) || (ints[(i + 1)].intValue() < store.getStore_credit()))
                continue;
            credit = i + 1;
            break;
        }
    ...
    public int generic_user_credit(String id) {
        ...
        for (int i = 0; i < ints.length - 1; ++i) {
            if ((ints[i].intValue() > user.getUser_credit()) || (ints[(i + 1)].intValue() < user.getUser_credit()))
                continue;
            credit = i + 1;
            break;
        }
