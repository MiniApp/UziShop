...
public class AlbumServiceImpl implements IAlbumService {
    ...
    public IPageList list(IQueryObject properties) {
        ...
        GenericPageList pList = new GenericPageList(Album.class, query, params, this.albumDao);
        if (properties != null) {
    ...
    public Album getDefaultAlbum(Long id) {
        ...
        params.put("album_default", Boolean.valueOf(true));
        List list = this.albumDao.query(
                "select obj from Album obj where obj.user.id=:user_id and obj.album_default=:album_default", params,
                -1, -1);
        if (list.size() > 0) {
            return ((Album) list.get(0));
        }
