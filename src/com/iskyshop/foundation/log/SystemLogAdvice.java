...
public class SystemLogAdvice {
    ...
    @AfterReturning("execution(* com.iskyshop.manage..*.*(..))&& @annotation(annotation)&&args(request,..) ")
    public void userLog(JoinPoint joinPoint, Log annotation, HttpServletRequest request) throws Exception {
    }

    @SuppressWarnings("unused")
    private void saveLog(JoinPoint joinPoint, Log annotation, HttpServletRequest request) throws Exception {
        String title = annotation.title();

        String methodName = joinPoint.getSignature().getName();

        String description = SecurityUserHolder.getCurrentUser().getTrueName() + "于"
        ...
        if (annotation.type().equals(LogType.REMOVE)) {
            ...
            if (ids.length > 1)
                description = description + "批量删除" + annotation.entityName() + "数据，Id为：" + mulitId;
            else
                description = description + "删除Id为：" + mulitId + "的" + annotation.entityName();
        }
        if (annotation.type().equals(LogType.SAVE)) {
            ...
            if (id.equals(""))
                ...
                description = description + "修改并更新Id为：" + id + "的" + annotation.entityName();
            }
        }
        if (annotation.type().equals(LogType.VIEW)) {
            String id = request.getParameter("id");
            description = description + "查阅Id为:" + id + "的" + annotation.entityName();
        }
