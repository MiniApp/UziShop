...
public class DatabaseTools implements IBackup {
    ...
    public boolean createSqlScript(HttpServletRequest request, String path, String name, String size, String tables)
        ...
        try {
            ...
            PrintWriter pwrite = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file, true), "UTF-8"),
                    true);
            ...
            for (String table : backup_list) {
                ...
                for (String line : list) {
                    ...
                    if (fsize > psize) {
                        ...
                        pwrite = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file, true), "UTF-8"),
                                true);
    ...
    public List<String> getAllDatas(String tableName) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                insert_sql.append("INSERT INTO " + tableName + " (" + typeStr.substring(0, typeStr.length() - 1)
                        + ") VALUES (");
                ...
                for (TableColumn dbColumn : columnList) {
                    ...
                    if (rs.getString(columnsFile) == null) {
                        ...
                        tempStr = tempStr.replaceAll("\"", "\\\"").replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n")
                                .replaceAll("<!--[\\w\\W\\r\\n]*?-->", "");
                        vector.add("'" + tempStr + "'");
                    } else {
                        vector.add("'" + rs.getString(columnsFile) + "'");
