...
public class ShopSecurityExceptionFilter extends SpringSecurityFilter implements InitializingBean {
    ...
    protected void sendStartAuthentication(ServletRequest request, ServletResponse response, FilterChain chain,
        ...
        if (this.logger.isDebugEnabled()) {
            this.logger
                    .debug("Authentication entry point being called; SavedRequest added to Session: " + savedRequest);
        }

        if (this.createSessionAllowed) {
            httpRequest.getSession().setAttribute("SPRING_SECURITY_SAVED_REQUEST_KEY", savedRequest);
        }
